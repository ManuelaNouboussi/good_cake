...
const Confiserie = () => {
  ...
  const handleRateRecipe = async (recipeId, rating) => {
    ...
    try {
      ...
      const result = await recipeFacade.getAllRecipes({ category: 'confiserie' });
      if (result && Array.isArray(result.data)) {
        setAllRecipes(result.data);
      } else if (Array.isArray(result)) {
        setAllRecipes(result);
  ...
  if (loading) {
    return (
      <div className="confiserie-page">
        <Header />
        <main className="confiserie-container">
          <JudicialHeader />
          <div className="loading-container">
            <div className="loading-spinner"></div>
            <p>Chargement des recettes de confiserie...</p>
          </div>
        </main>
      </div>
    );
        ...
        <main className="confiserie-container">
          ...
          <div className="search-and-filter-bar">
            <form className="search-form">
              <input
                type="text"
                placeholder="Rechercher une recette de confiserie..."
                value={searchQuery}
          ...
          {sortedRecipes.length === 0 && (
            <div className="no-recipes">
              ...
              <h3>
                {searchQuery.trim() !== '' 
                  ? 'Aucune recette de confiserie trouvée' 
                  : allRecipes.length === 0
                    ? 'Aucune recette de confiserie disponible'
                    : 'Aucune recette ne correspond à votre recherche'}
              </h3>
              <p>
                {searchQuery.trim() !== '' 
                  ? 'Essayez de modifier vos critères de recherche.' 
                  : 'Soyez le premier à ajouter une délicieuse recette de confiserie !'}
              </p>
              {searchQuery.trim() !== '' && (
                <button 
                  ...
                  <div className="recipe-modal__section">
                    ...
                    <div className="recipe-modal__video">
                      <a 
                        ...
                        className="recipe-modal__video-link"
                      >
